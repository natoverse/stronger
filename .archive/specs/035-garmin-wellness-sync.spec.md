...
- Lactate threshold, fitness age, max HR estimate (2026-09-17): Added three new metric groups, all fetched inside the existing hourly `garmin-wellness-sync.py` run rather than a separate slow-cadence workflow, since Garmin only exposes each as a single "latest"/profile snapshot rather than a per-day history (so every run just re-fetches the current value):
  - Running lactate threshold — `lactateThresholdHr` (bpm), `lactateThresholdSpeed` (m/s), `lactateThresholdPower` (watts), sourced from `get_lactate_threshold(latest=True)`'s combined `speed_and_heart_rate` + `power` payload. The Wellness page renders pace by converting the stored m/s value to `M:SS /km` at display time rather than storing pace directly, matching Garmin's own presentation.
  - Fitness Age — `fitnessAge` (years), sourced from `get_fitnessage_data(cdate)`, tolerant of both a nested `{"fitnessAge": {"age": ...}}` shape and a direct scalar.
  - Estimated max heart rate — `maxHrEstimate` (bpm), explicitly the physiological-ceiling value Garmin uses for heart-rate zone calculation (from `get_userprofile_settings().userData.maxHeartRate`), not the day's device-observed peak heart rate (`get_user_summary().maxHeartRate`), which was deliberately not added since it is a raw daily measurement rather than a fitness estimate.
  - All five fields render as simple accent-colored dot charts (no Garmin-defined threshold bands exist for these metrics) in the Training section (lactate threshold, fitness age) and Recovery section (max HR estimate, next to Resting Heart Rate).
- Lactate threshold history + max HR extraction correction (2026-09-18): Garmin does expose date-range lactate threshold series. Each wellness date now queries `get_lactate_threshold(latest=False, start_date=cdate, end_date=cdate, aggregation="daily")` and stores that day's heart rate, speed, and running power instead of repeating the latest snapshot. Estimated max HR now reads `maxHeartRateUsed` from the default heart-rate zone in `get_userprofile_settings().heartRateZones`, with the dedicated heart-rate-zones endpoint and running zone as fallbacks; the earlier assumed `userData.maxHeartRate` shape remains supported for compatibility.
- Imperial lactate threshold pace (2026-09-18): The Wellness page converts stored lactate-threshold speed from m/s to rounded `M:SS /mi` pace at display time. Other user-facing distance, elevation, altitude, and body-mass values were confirmed to already use miles, feet, and pounds respectively; metric storage units and the physiological VO₂ max unit remain unchanged.
- Lactate threshold source-unit correction (2026-09-18): The biometric `lactateThresholdSpeed/range` endpoint returns speed scaled to one tenth of m/s, not m/s. Normalize by multiplying by 10 **before rounding and persisting**; the frontend continues to convert canonical m/s to rounded seconds per mile. For example, raw `0.319` becomes `3.19` m/s and displays `8:24 /mi`, not `84:05 /mi`. The previous claim that small range values were stale placeholders was incorrect; remove the 20-minute/mile cutoff rather than hide valid source data. Reject only missing, nonnumeric, nonfinite, or nonpositive source speeds. Evidence: [a published API-to-Garmin pace comparison and conversion](https://github.com/arpanghosh8453/garmin-grafana/pull/120#issuecomment-2889169153) and [recorded range response shape](https://github.com/llehouerou/go-garmin/blob/133782228645a17de99eb7f5dd0f3c8f841e394d/testdata/cassettes/biometric.yaml#L167-L175). Recognize the daily series' `from` date alongside existing date aliases. Regression tests cover source extraction through Firestore normalization, imperial formatting, precision, missing data, and unchanged HR/power. Existing stored speeds must be re-fetched rather than guessed or multiplied again in the browser: after deployment, scheduled overwrites repair the last 72 hours, and a manual Garmin Wellness Sync with `backfill=true` repairs older history.

### Sleep Schedule (2026-09-20)

- Persist `sleepStartTimestampLocal`, `sleepEndTimestampLocal`, `sleepStartTimestampGMT`, and `sleepEndTimestampGMT` from `get_sleep_data(date).dailySleepDTO` as millisecond numbers alongside the existing sleep metrics. Missing values are null; older documents may omit the fields.
- Garmin's Local timestamps encode wall-clock date/time as epoch-like milliseconds, not UTC instants. Decode them with UTC accessors without applying the server or viewer timezone. Keep the GMT instants separately; do not guess local times from GMT when the local fields are absent.
- Add **Sleep Schedule** above Sleep Duration, reusing the Body Battery floating-range renderer. Each complete, valid sleep window belongs to its recorded local wake-up date, even when it differs from the wellness entry date or UTC date. Missing, incomplete, reversed, or over-24-hour local windows leave gaps rather than zero bars.
- Use a continuous overnight axis and unwrap bedtime around noon (e.g. 11 PM = 23 hours, 1 AM = 25 hours). Preserve the local start/end interval so DST changes do not move the displayed endpoints. The default axis spans 6 PM through noon and expands for outlying windows. This is an overnight schedule chart, not a sleep-duration calculation.
- Day headers show the latest complete night's local start/end times; hover/touch shows the selected bucket's times. Weekly/monthly bars average both endpoints after unwrapping across midnight. Their header is a per-night weighted average for the visible range, not an average of bucket averages. Use 12-hour times with AM/PM throughout.
