...
- The Garmin wellness sync now derives its sheet column count from `len(HEADER)` instead of asserting a stale hard-coded `24`, so header changes no longer break script import or range calculation. The current shipped header has 23 columns (A:W).
