...
- Hourly cadence + 24-hour window (2026-07-22): The scheduled sync now defaults to a last-24-hours window (`ROLLING_DAYS = 2`, today + yesterday to cover midnight) instead of a 14-day rolling window, and runs hourly during Pacific daytime instead of once daily. GitHub cron is UTC-only, so the cron (`0 0-6,13-23 * * *`) fires on every UTC hour that can be Pacific daytime under either PST/PDT, and a "Pacific window" workflow step skips runs during the 23:00-05:59 Pacific overnight quiet period (manual `workflow_dispatch` runs always proceed and can still pass `--backfill`). If an hourly scheduled run fails (e.g. Garmin API rate limits), the workflow opens/comments a single deduplicated `garmin-sync`-labeled issue so the cron can be tuned.
