...
- **Login stability hardening**: GIS token requests reuse one initialized client during normal operation. A client is discarded after a silent timeout so its delayed callback cannot interfere with the next attempt, and overlapping silent and interactive requests use separate clients so their callbacks cannot collide. Concurrent silent refreshes share one request. Interactive sign-in is guarded against repeat clicks, popup failures include actionable guidance, and a failed or canceled popup stays on the sign-in screen so the next click retries sign-in directly instead of detouring through the generic connection retry.
- **Interactive sign-in timing**: The 20-second timeout now applies only to silent refreshes. User-initiated sign-in allows up to five minutes for Google account selection, password, two-factor authentication, or consent before recovering from an unresponsive GIS request.
- **Popup-close recovery**: GIS can spuriously report `popup_closed` immediately after a successful first-click login. Interactive sign-in now waits briefly and makes one bounded, silent token request to recover the new Google session without another click. If recovery is unavailable, the app returns to the sign-in button without claiming that the user canceled the flow.
- **Expired-session flow**: A 401 while reconnecting now clears the token and immediately presents the interactive sign-in button. The connection flow does not attempt a silent refresh and retry, avoiding an ineffective retry that required users to click twice.
- **Explicit sign-in restored**: Silent refresh and popup-close recovery were removed after proving slower and less reliable than the original flow. Access tokens are reused only until their Google-provided expiry (normally one hour); after that, the app immediately shows the sign-in button and one user-initiated GIS request handles the next login.
