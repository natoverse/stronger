...
The workflow runs hourly, on the hour, and can also be triggered manually. It's idempotent — re-runs won't create duplicate rows.
...
2. A GitHub Actions workflow runs `scripts/garmin-sync.py` on an hourly schedule.
...
After verifying, the hourly cron will keep it updated automatically.

## Running on your own machine instead

If you'd rather not use GitHub Actions, run the script from any machine with the same three environment variables set:
