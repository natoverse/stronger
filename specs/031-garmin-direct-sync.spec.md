...
- **History backfill:** the daily sync only fetches the most recent activities (`get_activities(0, ACTIVITY_LIMIT)`), which is all the cron needs. To load older history, `scripts/garmin-sync.py` now reads an optional `GARMIN_SYNC_START_DATE` (`YYYY-MM-DD`) env var: when set, it fetches every activity on/after that date via `get_activities_by_date(start, today)`. Garmin Connect keeps full history, so there's no hard limit on how far back this reaches (e.g. `2021-01-01` pulls everything since 2021 inclusive). Dedup by activity ID keeps backfills idempotent. `garmin-sync.yml` exposes this as an optional `start_date` `workflow_dispatch` input for one-off manual backfills. See `GARMIN_SYNC_SETUP.md` → "Backfilling older history".
