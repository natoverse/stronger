...
After the Garmin activity sync succeeds, a nightly, opt-in sync finds new activities whose Garmin `activityType.typeKey` is `hiking` or `mountaineering`. It downloads each activity's GPX export, verifies that it contains track points with latitude and longitude, and imports valid tracks into a configured Gaia folder. Other activity types and activities without coordinates are skipped and reported.

Garmin coordinates are feasible: Garmin's Activity API documents FIT/GPX/TCX activity files, and the already-used unofficial `python-garminconnect` client exposes authenticated GPX downloads by activity ID. GPX is the safest interchange format because Gaia officially accepts it. The sync reuses `GARMIN_TOKENS`; its Gaia folder and credential are separate configuration.
...
- The installed client family exposes `download_activity(..., ActivityDownloadFormat.GPX)` against Garmin Connect's GPX export endpoint: https://github.com/cyberjunky/python-garminconnect/blob/981d150caeda7d632224a75f3895c08df27a2a34/garminconnect/__init__.py
...
- Evidence for the only known automated route is explicitly unofficial: `gaiagpsclient` says Gaia has no published API, uses reverse-engineered browser behavior, and requires a browser-extracted `sessionid` that must be replaced after expiry: https://github.com/kk7ds/gaiagpsclient and https://github.com/kk7ds/gaiagpsclient/commit/22d2431ddd3cf30a07e4c68f665bfe5f031bbb7f
